...
func _make_hud() -> void:
	...
	var hint := Label.new()
	hint.text = "W/S 前后 · A/D 转向 · 右键环视 · 滚轮缩放 · Tab 观景\nSai：Shift 下蹲 · R 重置   MD：左 Shift + W 加速（可同时 A/D 转向） · 松开 Shift 普通行走\nMD：Y 蹲起 · G 捡地 · K/L 踢击 · R 前滚 · B 目标 · 0 重置"
...
func _process(delta: float) -> void:
	...
	if not _headless:
		...
		label.text = "小小维修站 / %s\n%s" % [active_robot.to_upper(),_stage_label(str(actor.command.get("stage","就绪"))) if active_robot=="sai" else actor.SKILL_LABELS.get("sprint" if actor.brain.sprinting else actor.brain.policy,actor.brain.policy)]
