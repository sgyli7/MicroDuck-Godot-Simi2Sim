...
func build(workshop: Node3D, visible_geometry: bool, ball_mass: float=REFERENCE_BALL_MASS) -> void:
...
func place_target(position: Vector3) -> void:
	...
	body.global_transform=Transform3D(spawn.basis,Vector3(position.x,spawn.origin.y,position.z))
