...
func build(station:Node3D) -> void:
	...
	for band in range(rings.size()-1):
		...
		for i in range(160):
			...
			for v in [p,r,q,q,r,s]:st.add_vertex(v);terrain_faces.append(v)
		st.generate_normals();st.index()
		w._add(st.commit(),Vector3.ZERO,["silt","sand","rock","ochre","far_rock","distant","distant","distant"][band])
	...
	var observatory_base:=Vector3.ZERO
	for spec in [[Vector3(-36,1.7,-21),Vector3(4.6,5.2,2.8),.3],
		[Vector3(-31,1.2,4),Vector3(3.2,3.2,2.6),1.7],
		...
		if spec[2]==.3:observatory_base=p+Vector3(1.,spec[1].y-.2,.6)
	_observatory(observatory_base)
...
func ridge(a:float,r:float,band:int) -> Vector3:
	...
	return Vector3(dx*square*24.1*r*warp,y,dz*square*20.1*r*warp-6.)
...
static func outcrop(station:Node3D,p:Vector3,size:Vector3,seed:float,mesa:bool=false) -> void:
	var count:=13
	...
	for j in range(levels.size()-1):
		...
		st.generate_normals();st.index();station._add(st.commit(),Vector3.ZERO,(["silt","rock","sand","rock","chalk"] if mesa else ["silt","rock","chalk","rock","chalk"])[j])
		if j in [1,3]:
			for i in range(count):
	...
	if station.solid_scope:
		hull.generate_normals();hull.index();var rock_mesh:=hull.commit()
		station._solid(rock_mesh,Vector3.ZERO)
