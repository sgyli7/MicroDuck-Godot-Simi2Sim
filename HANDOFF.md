...
> **当前工作：行走加速（2026-09-12）**。用户确认只给行走增加左 Shift+W，边加速边 A/D，细节自主决定。会话 `results/sprint_20260912/`，从 11:49 开工，实际工时预算 4 小时、收尾保留 45 分钟；新开发种子 919000–919007，最终种子 920000–920029 尚未使用。协议见 [PROTOCOL](docs/sprint_20260912/PROTOCOL.md)，[研究与实验](docs/sprint_20260912/EXPERIMENTS.md)。
> 已提交原生加速入口和 GPU 训练代理；s05 完整开发集 **55/64、无跌倒**，长直行约 **0.220 vs 0.168 m/s**，仍有方向／交接／停止失败，**未晋级**。预览包 `dist/MicroDuck-ARM64-20260912-sprint-preview/` 已通过十模型真实／随机／边界自检，九模型哈希保持原值。宿主锁屏，键盘验收待完成。
> 此段为进行中检查点：s06 部署契约适配仍由工时监督器运行。恢复时先读账本、`supervisors/`、完成标记及实时进程；不要重复启动。失效的动作混合、保留策略和切换时重新捕获朝向试验已记录拒绝。最后结果形成后更新此段。
