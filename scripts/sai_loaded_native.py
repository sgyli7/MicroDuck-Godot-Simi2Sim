...
def run(out,kind='rough',seed=47,mass=.1,parameters=None,clamped=False,drive_speed=.5,turn_rate=0.):
    ...
    with socket.socket() as listener,ThreadPoolExecutor(max_workers=1) as pool:
        ...
        cmd=['godot','--headless','--fixed-fps','2000','--path',str(runtime),'res://probe.tscn','--',f'--port={listener.getsockname()[1]}',
    ...
    report=dict(drive_speed=drive_speed,turn_rate=turn_rate,kind=kind,seed=seed,mass=mass,clamped=clamped,parameters=parameters,metrics=m,engine=raw['engine'],physics_hz=2000,controller_hz=50)
...
def main():
    ...
    p.add_argument('--mass',type=float,default=.1);p.add_argument('--profile',type=Path);p.add_argument('--clamped',action='store_true');a=p.parse_args()
    parameters=json.loads(a.profile.read_text())['parameters'] if a.profile else None
    run(a.out,a.kind,a.seed,a.mass,parameters,a.clamped)
