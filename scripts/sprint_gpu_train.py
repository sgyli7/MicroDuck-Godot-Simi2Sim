...
Ordinary/idle actions always come from S05; only the sprint residual learns.
...
def main():
 ...
 p.add_argument('--constraints',choices=['none','positive','cat'],default='none');p.add_argument('--cuda-graphs',action='store_true');p.add_argument('--seed',type=int,default=951101);p.add_argument('--resume',type=Path);a=p.parse_args()
 ...
 def reward(requested,action,previous):
 ...
 files=[Path(__file__),Path('scripts/sprint_gpu_world.py'),Path('scripts/sprint_gpu_proxy.py'),Path('src/sim2sim/research/torch_anchor.py'),Path('src/sim2sim/research/torch_walking.py'),Path('src/sim2sim/research/models.py'),Path('src/sim2sim/research/constraint_returns.py'),Path('src/sim2sim/research/sprint_constraints.py')]
 config=dict(source=str(source),template=str(template),template_sha256=hashlib.sha256(template.read_bytes()).hexdigest(),source_sha256=anchor.sha256,constraints=a.constraints,constraint_probability_schedule='0.05 to0.25 over128 iterations, EMA .95' if a.constraints=='cat' else 'zero',variant='residual',std=.02,bound=.2,feet=a.feet,cuda_graphs=a.cuda_graphs,iterations=a.iterations,envs=a.envs,steps=a.steps,seed=a.seed,control=str(control),control_sha256=hashlib.sha256(control.read_bytes()).hexdigest(),code_sha256={str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in files},device=torch.cuda.get_device_name(),collection_device='cuda',learner_device='cuda',ordinary_controller='frozen S05',actor_mask='actual sprint selection only',reward='native_progress_v1',gamma=.99,lam=.95,epochs=4,minibatch=4096,actor_lr=1e-4,critic_lr=3e-4,target_kl=.008,critic_warmup=2,reset='uniform yaw and joint noise +/- .015 rad, clipped joint limits',task_termination='finite keyboard episode ends after final stop; no continuing-task bootstrap',game_physics_changed=False,foot_hulls=world.feet,inertia=world.inertia,torch=torch.__version__,cuda=torch.version.cuda,started_unix=time.time())
 atomic_json(a.output/'config.json',config)
 initial_iteration=0
 if a.resume:
  old=torch.load(a.resume,map_location='cuda',weights_only=False)
  for key in ['source_sha256','template_sha256','variant','std','bound','feet','cuda_graphs','envs','steps','seed','control_sha256','code_sha256','reward','constraints']:
  ...
  for iteration in range(initial_iteration+1,a.iterations+1):
   ...
   with torch.no_grad():
    for step in range(a.steps):
     base=anchor(obs);dist=policy.distribution(obs,base);action=torch.where(sprint[:,None],dist.sample(),base);value=critic(cobs)
     ...
     rew,fell,detail=reward(requested,action,previous);terms.append(torch.stack(list(detail.values())))
     phase+=1;finished=phase>=lengths[program];done=fell|finished
     for k,v in [('obs',obs),('cobs',cobs),('anchor',base),('action',action),('mean',dist.mean),('value',value),('logprob',dist.log_prob(action).sum(-1)),('reward',rew),('done',done),('mask',sprint)]:buffer[k].append(v.clone())
   ...
   torch.cuda.synchronize();collection_s=time.monotonic()-collect_start;learn_start=time.monotonic();before=copy.deepcopy(policy.state_dict());before_optimizer=copy.deepcopy(ao.state_dict());updates=0;stop=False
