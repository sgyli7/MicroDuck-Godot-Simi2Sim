...
class LoadedTerrain(Terrain):
    ...
    def function(self,x,y):
        ...
        if self.kind in ('up20','down20','up40','down40','flat'):return np.zeros_like(x+y)
    ...
    def stairs(self,p):
        if self.kind not in ('up20','down20','up40','down40','mixed20','mixed40'):return np.zeros(len(p))
        rise=.04 if '40' in self.kind else .02
    ...
    def model(self):
        ...
        if self.kind.startswith(('up','down','mixed')):
            start=1.1 if self.kind.startswith('mixed') else .45;rise=.04 if '40' in self.kind else .02
        ...
        ET.SubElement(obj,'geom',name='payload',type='box',size='.02 .015 .02',mass=str(self.mass),contype='4',conaffinity='3',group='4')
        model=mujoco.MjModel.from_xml_string(ET.tostring(xml,encoding='unicode'));model.hfield_data[:]=((self.z-low)/span).ravel()
        return model


def run(kind='rough',seed=47,mass=.1,parameters=None,out=None,seconds=None,*,clamped=False,yaw=0.,start_x=0.,tread=.18,terrain_scale=1.,drive_speed=.5,turn_rate=0.,crouch=0.):
    ...
    c=CompliantController(resource_root(),parameters)
    ...
    wheels=[model.body(n+'_wheel').id for n in ('front_left','front_right','rear_left','rear_right')];chassis=model.body('chassis').id
    ...
    started=time.monotonic();result=None;ground=[initial]*4;cargo_supported=False;bilateral=False
    for step in range(round(duration/dt)+1):
        ...
        cargo_supported=False;pad_forces=np.zeros(2)
        for index,ct in enumerate(data.contact):
            if payload_geom in ct.geom and t>=1.5:
                mujoco.mj_contactForce(model,data,index,contact_force);max_force=max(max_force,float(contact_force[0]))
        ...
        if step%substeps==0:
            ...
            finish_x=(1.1 if kind.startswith('mixed') else .45)+3*tread+.20
            ...
            state=dict(robot_id='Sai_Agent_001',physics_owner='Godot/Jolt',time=t,q=data.qpos[qa].tolist(),v=data.qvel[va].tolist(),
                ...
                command=[speed,turn_rate if moving else 0.,crouch],terrain_heights=scan(SCAN),terrain_path_heights=scan(PATH),terrain_edge_heights=scan(DENSE),wheel_ground_heights=ground)
            result=c.command(state)
            trace.append(dict(time=t,position=data.qpos[:3].tolist(),cargo_local=cargo_local.tolist(),speed=float((rot.T@data.qvel[:3])[0]),command=speed,stage=result['stage'],upright=float(rot[2,2]),qpos=data.qpos.tolist()))
...
def main():
    ...
    p.add_argument('--mass',type=float,default=.1);p.add_argument('--parameters',type=float,nargs=6);a=p.parse_args()
    print(json.dumps(run(a.kind,a.seed,a.mass,a.parameters,a.out),indent=2),flush=True)
