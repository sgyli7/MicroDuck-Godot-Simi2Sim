...
def adapt(source,dest,matrix=None,bias=None,task="walking"):
    ...
    nodes += [helper.make_node("MatMul",["command","command_matrix"],["mapped"]),
        ...
        helper.make_node("Concat",["before","latent_command","after"],[m.graph.input[0].name],axis=1)]
    ...
    for k,v in dict(sim2sim_transform="affine_command_conditioning",sim2sim_task=task,
        ...
        sim2sim_command_adapter=json.dumps(dict(matrix=matrix.tolist(),bias=bias.tolist()))).items():
...
def parity(source,adapted,matrix,bias=None,n=10000):
