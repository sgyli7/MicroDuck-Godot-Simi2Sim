...
def command(arguments,timeout=180):
    subprocess.run([str(value) for value in arguments],check=True,timeout=timeout)


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--jobs',type=int,default=8)
    ...
    if not bindings.exists():
        ...
        command(['git','clone','--depth','1','--branch',PIN['godot_cpp']['tag'],PIN['godot_cpp']['url'],bindings])
    ...
    for target,directory in [('template_debug','build'),('template_release','build-release')]:
